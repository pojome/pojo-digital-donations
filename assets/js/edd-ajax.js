...
jQuery( document ).ready( function( $ ) {
	...
	$( 'body' ).on( 'click.eddRemoveFromCart', '.edd-remove-from-cart', function( event ) {
		...
		.on( 'click.eddAddToCart', '.edd-add-to-cart', function( e ) {
	...
	$( 'div.edd_price_options :input' ).on( 'change', function() {
		var $this_form = $( this ).closest( 'form.edd_download_purchase_form' ),
			$custom_amount_radio = $( 'input.edd_custom_amount_radio', $this_form );

		if ( $custom_amount_radio.prop( 'checked' ) ) {
			$( 'div.edd-custom-amount-wrapper', $this_form ).fadeIn();
		} else {
			$( 'div.edd-custom-amount-wrapper', $this_form ).fadeOut( 'fast' );
		}
	} )
		.trigger( 'change' );

	$( '.edd-add-to-cart' ).on( 'click', function( e ) {
		$( 'div.edd_errors.edd_custom_amount_error' ).remove();
		
		var $this_form = $( this ).closest( 'form.edd_download_purchase_form' ),
			$custom_amount_radio = $( 'input.edd_custom_amount_radio', $this_form ),
			$custom_amount = $( 'input.edd_custom_amount', $this_form );
		...
		$( $this_form ).append( '<div class="edd_errors edd_custom_amount_error"><p class="edd_error">Please enter a custom price higher than the minimum amount.</p></div>' );
...
function edd_load_gateway( payment_mode ) {
	...
	jQuery.post( edd_scripts.ajaxurl + '?payment-mode=' + payment_mode, {
