 ...
 *
 * Hooks the pdd_set_upload_dir filter when appropriate. This function works by
 * hooking on the WordPress Media Uploader and moving the uploading files that
 * are used for PDD to an pdd directory under wp-content/uploads/ therefore,
 * the new directory is wp-content/uploads/pdd/{year}/{month}. This directory is
 ...
 * @return void
 */
function pdd_change_downloads_upload_dir() {
	global $pagenow;
...
add_action( 'admin_init', 'pdd_change_downloads_upload_dir', 999 );
...
function pdd_create_protection_files( $force = false, $method = false ) {
	if ( false === get_transient( 'pdd_check_protection_files' ) || $force ) {

		$upload_path = pdd_get_upload_dir();

		// Make sure the /pdd folder is created
		wp_mkdir_p( $upload_path );

		// Top level .htaccess file
		$rules = pdd_get_htaccess_rules( $method );
		if ( pdd_htaccess_exists() ) {
		...
		$folders = pdd_scan_folders( $upload_path );
		...
		set_transient( 'pdd_check_protection_files', true, 3600 * 24 );
	}
}
add_action( 'admin_init', 'pdd_create_protection_files' );

/**
 * Checks if the .htaccess file exists in wp-content/uploads/pdd
 *
 * @since 1.8
 * @return bool
 */
function pdd_htaccess_exists() {
	$upload_path = pdd_get_upload_dir();
...
/**
 * Scans all folders inside of /uploads/pdd
 *
 * @since 1.1.5
 * @return array $return List of files inside directory
 */
function pdd_scan_folders( $path = '', $return = array() ) {
	...
	if ( ! empty( $lists ) ) {
		foreach ( $lists as $f ) {
			if ( is_dir( $path . DIRECTORY_SEPARATOR . $f ) && $f != "." && $f != ".." ) {
				...
				pdd_scan_folders( $path . DIRECTORY_SEPARATOR . $f, $return);
			}
		}
	}
...
/**
 * Retrieve the .htaccess rules to wp-content/uploads/pdd/
 *
 * @since 1.6
 *
 * @param bool $method
 ...
 */
function pdd_get_htaccess_rules( $method = false ) {
	...
	endswitch;
	$rules = apply_filters( 'pdd_protected_directory_htaccess_rules', $rules, $method );
	return $rules;
}
