 ...
 * @author Sunny Ratilal
 */
function pdd_generate_pdf( $data ) {
	$pdd_pdf_reports_nonce = $_GET['_wpnonce'];

	if ( wp_verify_nonce( $pdd_pdf_reports_nonce, 'pdd_generate_pdf' ) ) {
		require_once PDD_PLUGIN_DIR . '/includes/libraries/fpdf/fpdf.php';
		require_once PDD_PLUGIN_DIR . '/includes/libraries/fpdf/pdd_pdf.php';

		$daterange = date_i18n( get_option( 'date_format' ), mktime( 0, 0, 0, 1, 1, date( 'Y' ) ) ) . ' ' . utf8_decode( __( 'to', 'pdd' ) ) . ' ' . date_i18n( get_option( 'date_format' ) );
		...
		$pdf->Cell( 0, 3, utf8_decode( __( 'Sales and earnings reports for the current year for all products', 'pdd' ) ), 0, 2, 'L', false );

		$pdf->SetFont( 'Helvetica', '', 13 );
		$pdf->Ln();
		$pdf->SetTextColor( 150, 150, 150 );
		$pdf->Cell( 0, 6, utf8_decode( __( 'Date Range: ', 'pdd' ) ) . $daterange, 0, 2, 'L', false );
		$pdf->Ln();
		$pdf->SetTextColor( 50, 50, 50 );
		$pdf->SetFont( 'Helvetica', '', 14 );
		$pdf->Cell( 0, 10, utf8_decode( __( 'Table View', 'pdd' ) ), 0, 2, 'L', false );
		$pdf->SetFont( 'Helvetica', '', 12 );

		$pdf->SetFillColor( 238, 238, 238 );
		$pdf->Cell( 70, 6, utf8_decode( __( 'Product Name', 'pdd' ) ), 1, 0, 'L', true );
		$pdf->Cell( 30, 6, utf8_decode( __( 'Price', 'pdd' ) ), 1, 0, 'L', true );
		$pdf->Cell( 50, 6, utf8_decode( __( 'Categories', 'pdd' ) ), 1, 0, 'L', true );
		$pdf->Cell( 50, 6, utf8_decode( __( 'Tags', 'pdd' ) ), 1, 0, 'L', true );
		$pdf->Cell( 45, 6, utf8_decode( __( 'Number of Sales', 'pdd' ) ), 1, 0, 'L', true );
		...
		if ( $downloads ):
			...
			foreach ( $downloads as $download ):
				...
				if ( pdd_has_variable_prices( $download->ID ) ) {

					$prices = pdd_get_variable_prices( $download->ID );
					...
					$price = html_entity_decode( pdd_currency_filter( pdd_format_amount( $min ) ) . ' - ' . pdd_currency_filter( pdd_format_amount( $max ) ) );
				} else {
				...
				$earnings = html_entity_decode ( pdd_currency_filter( pdd_get_download_earnings_stats( $download->ID ) ) );
			...
			$title = utf8_decode( sprintf( __( 'No %s found.', 'pdd' ), pdd_get_label_plural() ) );
			$pdf->Row( array( $title ) );
		endif;
		...
		$pdf->SetTextColor( 50, 50, 50 );
		$pdf->SetFont( 'Helvetica', '', 14 );
		$pdf->Cell( 0, 10, utf8_decode( __('Graph View', 'pdd') ), 0, 2, 'L', false );
		$pdf->SetFont( 'Helvetica', '', 12 );

		$image = html_entity_decode( urldecode( pdd_draw_chart_image() ) );
		$image = str_replace( ' ', '%20', $image );

		$pdf->SetX( 25 );
		$pdf->Image( $image .'&file=.png' );
		$pdf->Ln( 7 );
		$pdf->Output( apply_filters( 'pdd_sales_earnings_pdf_export_filename', 'pdd-report-' . date_i18n('Y-m-d') ) . '.pdf', 'D' );
	}
}
add_action( 'pdd_generate_pdf', 'pdd_generate_pdf' );
 ...
 */
function pdd_draw_chart_image() {
	require_once PDD_PLUGIN_DIR . '/includes/libraries/googlechartlib/GoogleChart.php';
	require_once PDD_PLUGIN_DIR . '/includes/libraries/googlechartlib/markers/GoogleChartShapeMarker.php';
	require_once PDD_PLUGIN_DIR . '/includes/libraries/googlechartlib/markers/GoogleChartTextMarker.php';
	...
	while ( $i <= 12 ) :
		$earnings .= pdd_get_earnings_by_date( null, $i, date('Y') ) . ",";
		$sales .= pdd_get_sales_by_date( null, $i, date('Y') ) . ",";
		$i++;
	endwhile;
	...
	$data->setLegend( __( 'Earnings', 'pdd' ) );
	...
	$data->setLegend( __( 'Sales', 'pdd' ) );
	$data->setColor( 'ff6c1c' );
	$chart->addData( $data );

	$chart->setTitle( __( 'Sales and Earnings by Month for all Products', 'pdd' ), '336699', 18 );
