...
?>
	<form id="<?php echo $form_id; ?>" class="edd_download_purchase_form" method="post">
		...
		<?php if ( edd_has_custom_amount( $args['download_id'] ) ) :
			$min_amount = get_post_meta( $args['download_id'], 'edd_min_custom_amount', true );
			$have_min_amount = ! empty( $min_amount ) && '0' !== $min_amount;
			?>
			<div class="edd-custom-amount-wrapper">
				<?php _e( 'Enter a amount you\'d like to pay:', 'edd' ); ?>
				<?php echo edd_currency_filter( '' ); ?>
				<input type="text" value="" name="edd_custom_amount" class="edd_custom_amount" size="1"<?php if ( $have_min_amount ) echo ' data-min_amount="' . $min_amount . '"'; ?> />
				<?php if ( $have_min_amount ) : ?>
					(<?php _e( 'Min', 'edd' ); ?> <?php echo edd_currency_filter( '' ); ?><?php echo $min_amount; ?>)
				<?php endif; ?>
			</div>
		<?php endif; ?>
...
function edd_purchase_variable_pricing( $download_id = 0 ) {
	...
	<div class="edd_price_options">
		<ul>
			...
			if ( edd_has_custom_amount( $download_id ) ) : ?>
				<li>
					<label>
						<input type="<?php echo $type; ?>" name="edd_options[price_id][]" value="<?php echo esc_attr( $custom_amount_key ); ?>" class="<?php echo esc_attr( 'edd_price_option_' . $download_id ); ?> edd_custom_amount_radio" />
						<?php _e( 'Name your price', 'edd' ); ?>
					</label>
				</li>
			<?php endif;
